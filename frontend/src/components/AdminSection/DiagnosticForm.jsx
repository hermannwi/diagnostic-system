...
export default function DiagnosticForm(props) {
    ...
    const questionObject = props.allQuestions.find(q => q.id.toString() === selectedId)
    if (questionObject) {
        setQuestion(questionObject.question)  
    }
}
    // function showQuestions() {
    //     const systemObject = props.allSystems.find(question => system.id.toString() === selectedSystem)
    //     return systemObject?.
    // }
    ...
    function showParts() {
        const productObject = props.allProducts.find(product => product.id.toString() === selectedProduct)
        return productObject?.parts.map(part => {
                        console.log(product)
                        return (<option key={part.id} value={part.id}>
                            {part.part}
                        </option>)
                    })
    }
    ...
    <section className="diagnostic-form">
        ...
        <form onSubmit={handleSubmit} method='post'>
            ...
            <select className='select-question' name="select-question" id="select-question"
                    value={selectedQuestion} 
                    onChange={handleQuestionChange}>
                    <option value=''>Select an existing question...</option>
                    {props.allQuestions.map(question => {
                        return (<option key={question.id} value={question.id}>
                            {question.question}
                        </option>)
                    })}
